...
class aibot_3:
    ...
    def __init__(self):
        ...
        self.name = f"aibot_{aibot_3.count}"
        aibot_3.count += 1
        self.max_depth = 2 # 2 is like the max for the aibot to not run out of time
        
        self.transposition_table = {}

    def mine(self, board: Board, color: Space) -> Optional[tuple[Coordinate, Coordinate]]:
        best_mine = None
        
        for depth in range(1, self.max_depth + 1):
            score, mine_found = self.minimax_ab(board, color, depth, alpha=-float('inf'), beta=float('inf'), maximizing=True) #Start from here
            if mine_found is not None:
                best_move = mine_found
    ...
    def minimax_ab(self, board: Board, color: Space, depth: int, alpha: float, beta: float, maximizing: bool, movesmine: bool):
        ...
        if movesmine is True:
            moves = self.possible_moves(board, current_color)
        else: 
            moves = board.mineable_by_player(current_color)
        ...
        if maximizing: #Max
            ...
            for move in moves:
                ...
                evaluation, _ = self.minimax_ab(new_board, color, depth - 1, alpha, beta, maximizing=False, movesmine) 
